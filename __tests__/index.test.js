...
test("task4", () => {
  ...
  // assert.equal(
  //   schema2.isValid([1, [2], [1, [2, [3, [4, [5, [6, [7, [8]]]]]]]]]),
  //   false
  // );

  assert.equal(
    schema3.isValid([1, 2, 3, [1, [2], [1, [2]], [1, [2, [3, [4]]]]]]),
    false
  );
...
test("task5", () => {
  ...
  const schema1 = validator.object().shape({
    ...
    basket: validator.array().maxDepth(2),
  });
  ...
  assert.equal(
    schema1.isValid({
      name: "B11",
      basket: ["potatos", "tomatos", "oranges", ["carrots"]],
    }),
    true
  );
  assert.equal(
    schema1.isValid({ name: "12", basket: ["potatos", "tomatos", "oranges"] }),
    false
  );
  assert.equal(schema1.isValid({}), false);
  ...
  assert.equal(
    schema2.isValid({
      name: "sergey",
      basket: ["apple", "cucumber"],
      payment: ["10 dollars", "10 cents", [0, [1]]],
    }),
    true
  );
  assert.equal(
